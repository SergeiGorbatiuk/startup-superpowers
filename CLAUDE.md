...
Public product guide, live at [docs.startupsuperpowers.io](https://docs.startupsuperpowers.io), built with [Mintlify](https://mintlify.com) and deployed from this repo: `docs-site/docs.json` is the site config (name, colors, logo, navigation) and the `.mdx` files are the pages. The Mintlify GitHub app watches `main` and redeploys on push — no CI or build artifacts in the repo. Preview locally with `cd docs-site && npx mint dev`; check links with `npx mint broken-links`.

Current structure: Get Started (`introduction`, `installation`, `quickstart`), Core Concepts (`concepts/workspace`, `concepts/plan`, `concepts/evidence-graph`, `concepts/advisor`), Skills (`skills/overview` + one page per skill). Skill pages follow a shared template: purpose paragraph → "When to reach for it" → one section per workflow → "What it writes" table → "Good to know" gotchas → related-links cards. Docs are founder-facing only — internals (subagents, hooks, layers) appear only as their founder-visible value (e.g. "bias-isolated analysis"), never as architecture. When a change alters founder-facing behavior — install steps, skill set, workflows, artifact conventions a founder sees — update the corresponding `docs-site/` page in the same PR. New pages must be added to the `navigation` block in `docs.json` or they won't appear on the site.
